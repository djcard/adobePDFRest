component accessors="true" {
	...
	property name="credentialMethod" inject="coldbox:setting:credentialMethod@adobePDFRest" default="jwt";
	property name="oauthScopes"    inject="coldbox:setting:scopes@adobePDFRest";
	...
	function onDICOmplete(){
		...
		if ( getcredentialMethod() == "Oauth" ) {
			gettokenoauth();
		} else {
			setJWTToken( createJWT() );
			if ( getJWTToken().len() == 0 ) {
				writeDump( "THERE WAS NO JWT" );
				return this;
			}
			obtainApiToken();
		}
	...
	/***
	 * Obtains an access token using the two legged Oauth protocol. For more info see:
